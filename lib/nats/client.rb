...
module NATS
      ...
      when AWAITING_CONTROL_LINE
        ...
        when UNKNOWN
          if @server_info[:ssl_required]
            ...
            case @buf
            when ERR2
              # In case the TLS handshake was forced on us, we might not be able
              # to parse the protocol but can still do a best effort match anyway
              # in case the server has sent an error.
              current = server_pool.first
              current[:error_received] = true
              err_cb.call(NATS::ServerError.new($1))
              @buf = $'
              ...
              @buf = rest
              err_cb.call(NATS::ServerError.new("Unknown protocol: #{line}"))
            end
          else
            @buf = $'
  ...
  def process_connect #:nodoc:
    # Reset reconnect attempts looks like connection has been successful so far.
    current = server_pool.first
